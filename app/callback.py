...
def on_connect(client, userdata, flags, reason_code, properties):
    ...
    if reason_code == 0:
        ...
        client.subscribe(MQTT_CAR_ARRIVAL_TOPIC) # 도착 토픽 구독
        print(f"토픽 구독 시작: {c.MQTT_ORDER_TOPIC}, {MQTT_CAR_ARRIVAL_TOPIC}")
    else:
...
def on_message(client, userdata, msg):
    global car_arrived # 전역 변수 사용 선언
    ...
    # --- 자동차 도착 메시지 처리 로직 ---
    if msg.topic == MQTT_CAR_ARRIVAL_TOPIC:
        if raw_payload.upper() == "ARRIVED":
            car_arrived = True
            print(">> [CAR STATUS] 자동차 도착 메시지 수신 완료. 주문 처리를 시작합니다.")
        else: # ARRIVED가 아니면 (READY, DEPARTED 등)
            car_arrived = False
            print(">> [CAR STATUS] 자동차가 아직 도착하지 않았습니다. 주문 대기 상태 유지.")
        return 
    # --------------------------------------------------

    # --- 주문 메시지 처리 로직 시작 ---
    if msg.topic == c.MQTT_ORDER_TOPIC:
        
        if not car_arrived:
            print(">> [ORDER PAUSED] 자동차 도착 메시지를 받지 못했습니다. 주문 실행을 대기합니다.")
            return
